...
func processTimeRange(
        ...
        if duration > float64(maxMinutes) && duration > minSplitMinutes.Minutes() {
                step := time.Duration(duration/float64(splitFactor)) * time.Minute
                for i := 0; i < splitFactor; i++ {
                        ...
                        wg.Add(1)
                        go processTimeRange(query, s, e, outputDir, maxMinutes, dryRun, splitFactor, failed, failedMutex, depth+1, wg)
                }
                return
        }
        ...
        for attempt := 0; attempt < 5; attempt++ {
                ...
                if err != nil {
                        if strings.Contains(err.Error(), "429") {
                                logWarn("%s🚦 Rate limited on job creation. Retrying in %v...", prefix, apiRateLimitDelay)
                                time.Sleep(apiRateLimitDelay)
                                continue
                        }
                        break
        ...
        if err := waitForJobCompletion(jobID); err != nil {
                logError("%s❌ Job failed %s: %v", prefix, jobID, err)
                failedMutex.Lock()
                *failed = append(*failed, start)
                failedMutex.Unlock()
                return
        ...
        messages, err := fetchAllMessages(jobID)
        if err != nil {
                logError("%s❌ Failed to fetch messages: %v", prefix, err)
                failedMutex.Lock()
                *failed = append(*failed, start)
                failedMutex.Unlock()
                return
        ...
        if len(messages) >= messageLimit && duration > minSplitMinutes.Minutes() {
                ...
                } else if writeEmptyFiles && end.Sub(start) == 1*time.Minute {
        ...
        for year := args.YearRange[0]; year <= args.YearRange[1]; year++ {
                for month := args.MonthRange[0]; month <= args.MonthRange[1]; month++ {
                        ...
                        for day := startDay; day <= endDay; day++ {
                                ...
                                wg.Add(1)
                                go processTimeRange(args.Query, start, end, args.OutputDir, args.MaxMinutes, args.DryRun, args.SplitFactor, &failedTimeRanges, &failedMutex, 0, &wg)
                        }
                }
        }
