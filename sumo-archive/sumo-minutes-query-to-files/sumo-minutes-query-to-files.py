...
def write_minute_atomically(msgs: list, out_path: Path):
    ...
    try:
        ...
        logging.error(f"❌ Error writing to {out_path}: {e.__class__.__name__}: {e}")
...
def wait_for_job_completion(job_id: str):
    ...
    while True:
        ...
        if response.status_code == 404:
            ...
            # logging.warning("Search job {job_id} not found (possibly expired or canceled).")
        response.raise_for_status()
...
def fetch_all_messages(job_id: str) -> list:
    ...
    while True:
        ...
        if response.status_code == 404:
            ...
            # logging.warning("Search job {job_id} not found while fetching messages (possibly expired).")
        response.raise_for_status()
